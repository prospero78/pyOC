...
class тПроцедура:
	...
	def __ПроцедурыВнутр_Проверить(сам):
		...
		if len(сам.__слова_проц) > 0:
			...
			сам.__проц[len(сам.__проц)] = тПроцедура(парам)
			сам.__слова_проц = {}
	...
	def __Переменные_Проверить(сам):
		...
		print("После переработки переменных (НЕ ОТБРАСЫВАЕТ СЕКЦИЮ!!!!):")
		for ключ in сам.__слова_проц:
			print(сам.__слова_проц[ключ])
