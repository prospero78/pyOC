...
class тКонстанты:
	...
	def ЕслиКонстОграничен(сам):
		...
		"""
		тег_всего = len(сам.__слова_модуля)-1
		тег_счёт = 0 # первый слово после CONST, а сам CONST уже распознали и отбросили
		слово = сам.__слова_модуля[тег_счёт]
	...
	def Теги_Получить(сам):
		...
		"""
		слова = {}  # будущий словарь тегов
		for счёт_конст in range(0, сам.__слово_конец.номер+1): # CONST уже отброшено
			слово = сам.__слова_модуля[счёт_конст]
			слово._Номер_Уст(счёт_конст)
			#print("#", счёт_конст, "num",слово.номер, слово.имя)
			слова[счёт_конст] = слово
		сам.__слова_секции = слова

		слова = {}  # будущий словарь тегов
		счёт = 0
		for счёт_импорт in range(сам.__слово_конец.номер+1, len(сам.__слова_модуля)): # Пропускаем IMPORT
			слово = сам.__слова_модуля[счёт_импорт]
			слово._Номер_Уст(счёт)
			#print("+",счёт_импорт, слово.номер, слово.имя)
			слова[счёт] = слово
			счёт += 1
		сам.__слова_модуля = {}
		сам.__слова_модуля = слова
	...
	def Константы_Разбить(сам):
		...
		счёт = 0
		while счёт < len(сам.__слова_секции)-1:
			#print(счёт, len(сам.__слова_секции))
