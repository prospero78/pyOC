...
class тПеременные(тСекция):
	def __init__(сам, пМодуль):
		def Слова_Проверить():
			...
			стрОш = "В секцию переменных должен передаваться словарь слов, type=" + str(type(пМодуль.слова))
	...
	def __Обработать(сам):
		...
		if сам.__Слово_VAR_Обрезать():
			сам.__модуль.конс.Печать("Есть переменные!")
		if сам.__ЕслиПеремНеПустые():
			сам.__модуль.конс.Печать("Переменные не пустые!")
			#сам.__Перем_Печать()
