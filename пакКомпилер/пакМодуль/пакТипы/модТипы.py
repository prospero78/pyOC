...
class тТипы:
	...
	def __Типы_Разделить(сам):
		...
		def ИмяТипа_Проверить():
			счёт = 0
			счёт_цикла = 0
			while len(сам.__слова_секции) != 0 and счёт_цикла < 100:
				тип_имя = сам.__слова_секции[счёт]
				if not тип_имя.ЕслиИмя():
					сам.ошибка.Коорд("тТипы: Неправильное имя типа в определении", тип_имя.коорд, тип_имя.строка)
					...
					сам.__слова_секции = {}
					сам.__слова_секции = сам.__типы[типы_всего].слова_секции
				счёт_цикла += 1
			print("счёт_цикла типов =", счёт_цикла)
			сам.__счёт = счёт
			for i in сам.__слова_секции:
				print(сам.__слова_секции[i])
