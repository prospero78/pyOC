...
class тТипы(тСекция):
	def __init__(сам, пМодуль):
		def Слова_Проверить():
			бУсл = type(пМодуль.слова) == dict
			стрОш = "В секцию типов должен передаваться словарь слов, type=" + str(type(пМодуль.слова))
			пМодуль.конс.Проверить(бУсл, стрОш)

		тСекция.__init__(сам, пМодуль)
	...
	def __ЕслиТипыНеПустые(сам):
		...
		def Слово_Проверить():
			...
			модуль.конс.Проверить(бУсл, стрОш)
		модуль = сам.__модуль
	...
	def __Типы_Разделить(сам):
		...
		while len(сам.слова_секции) > 0:
			тип = None
			тип = тТип(сам.__модуль, сам.слова_секции)
	...
	def __Обработать(сам):
		...
		if сам.__Слово_TYPE_Обрезать():
			сам.__модуль.конс.Печать("Есть типы!")
		if сам.__ЕслиТипыНеПустые():
			сам.__модуль.конс.Печать("Типы не пустые!")
			#сам.Теги_Печать()
			...
			сам.__модуль.конс.Печать("Нет типов!")
