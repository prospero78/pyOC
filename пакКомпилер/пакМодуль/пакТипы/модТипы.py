...
class тТипы:
   ...
   def __ЕслиТипыПусто(self):
      ...
      маркер = (тег.слово == "VAR") or (тег.слово == "PROCEDURE") or (тег.слово == "BEGIN")
      бПусто = True
      ...
      def РодТипа():
         def Запись_База():
            def Тип_База(счёт, тег_база):
               ...
               """
               база_имя = ""
               if тег_база.слово == ";": # У типа нет базы
                  счёт += 1
               elif тег_база.слово == "(": # У типа есть база
                  счёт += 1
                  тег_база = self.__теги_типы[счёт]
                  база_имя = тег_база.слово
                  счёт += 1
                  тег_база = self.__теги_типы[счёт]
                  if тег_база.слово != ")":
            ...
            счёт += 1
            тег_база = self.__теги_типы[счёт]
            print(5, тег_база.слово)
            тег_база, счёт = Тип_База(счёт, тег_база)
            print(6, тег_эксп.слово)
         def Указатель_База():
            def ЕслиТО():
               счёт = self.__счёт
               счёт += 1
               тег_указ = self.__теги_типы[счёт]
               #print("Тег <ТО>", тег_указ.слово)
                  
               # Кючевое слово TO
               if тег_указ.слово != "TO":
            ...
            def ВилкаМодульТочкаТип():
               ...
               #print(9, " =тег_разд_база= '"+тег_разд_база.слово+"'")
               if тег_разд_база.слово == ".":
                  тег_база_модуль = тег_разд_база
                  счёт += 1
                  # имя базового типа в модуле
                  тег_база_тип = self.__теги_типы[счёт]
                  print("   Модуль     : "+тег_указ.слово+"'")
                  print("   Базовый тип: "+тег_база_тип.слово+"'")
               # Может быть уже разделитель типов
               elif тег_разд_база.слово == ";":
                  счёт += 1
                  тег_база_тип = self.__теги_типы[счёт]
         ...
         print("   Род типа:", тег_род.слово)
         if тег_род.слово in ["RECORD", "POINTER", "ARRAY"] or тег_род.ЕслиИмя():
            if тег_род.слово == "RECORD":
               Запись_База()
            elif тег_род.слово == "POINTER":
               Указатель_База()
            elif тег_род.слово == "ARRAY":
               ЕслиМассив()
