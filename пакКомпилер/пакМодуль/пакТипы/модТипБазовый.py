...
class тТипБазовый:
	def __init__(сам, пКорень, пСловаСекции):
		...
		сам.__стрИмя = ""
		сам.__род = тРод.сБезРода # Род типа -- встроенный, POINTER, ARRAY, RECORD, PROCEDURE
	...
	def ЗаписьКонец_Обрезать(сам):
		"""
		Здесь может встретиться два слова: "END" или имя поля.
		Имя поля может быть любым, поэтому падать на этой проверке нельзя.
		"""
		бВыход = False
		строка_конец = сам.Слово_Проверить()
	...
	def Разделитель_Обрезать(сам):
		"""
		В записях может встречаться как разделитель ";", так и
		продолжение описания типа с вложенными полями.
		Поэтому здесь assert не делаем.
		"""
		бРезультат = False
	...
	def __Экспорт_Проверить(сам):
		...
		if строка_экспорт == "*": # есть экспорт
			...
			assert False, "тТипБазовый: Символ экспорта допустим '*' или '=',    строка=" + строка_экспорт
