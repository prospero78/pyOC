...
class тТип:
	...
	def __Запись_Проверить(сам):
		...
		def СкобкаЛевая_Обрезать():
			...
			"""
			сам.__СловаСекции_Обрезать()
			print("===есть предок!===", сам.__слова_секции[0].строка)
		...
		if сам.__ЗаписьКонец_Обрезать:
			assert False, "Этого текста не может быть.\n Все пустые записи распознаны"
		else:
			assert False, "тТип: продолжение разбора сложной записи"
