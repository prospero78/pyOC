...
class тПроцедура(тПроцБазовая):
	def __init__(сам, пОберон, пДанные:dict)->None:
		...
		сам.ош = тОшибка(пОберон, "тПроцедура")

		тПроцБазовая.__init__(сам, пОберон, пДанные)

		сам.__слова_проц :dict= {} # Слова в своей процедуре
		сам.__бВозврат :bool = False # признак возврата значения
		сам.__тВозврат :str = "" # Тип возвращаемого значения
		сам.__проц_конец = None # указатель на окончание своей процедуры
		сам.__конст :dict= {} # Возможные константы в процедуре
		сам.__типы :dict= {}  # Возможные типы в процедуре
		сам.__перем :dict= {} # Возможные переменные в процедуре
		сам.__проц :dict= {} # Возможные процедуры в процедуре
		сам.параметры :dict= {} # словарь параметров передаваемых в процедуру
	...
	def __СловаПроц_Получить(сам)->None:
		"""
		Выбирает слова по своей процедуре.
		Дальше работает только с ними.
		"""
		слова_проц :dict= {}  # будущий словарь слов секции процедур
		цСчёт :int= 0
		ключи :list= []
	...
	def __КонецСвой_Найти(сам)->None:
		"""
		...
		бЕсть :bool= True
		while бЕсть:
			слово0 :тСлово= сам.слова_секции[цКонец]
			if слово0.строка =="END":
				слово1 :тСлово= сам.слова_секции[цКонец+1]
				if слово1.строка == сам.имя:
					слово2 :тСлово= сам.слова_секции[цКонец+2]
	...
	def __ПроцедурыВнутр_Проверить(сам)->None:
		"""
	...
	def __Переменные_Проверить(сам)->None:
		"""
	...
	def __Константы_Проверить(сам)->None:
		"""
	...
	def __Разделитель_Обрезать(сам)->None:
		...
		"""
		слово_раздел :тСлово= сам.слова_секции[0]
		строка_раздел :str= слово_раздел.Проверить()
		if строка_раздел == ";":
			сам.СловаСекции_Обрезать()
		else:
			assert False, "тПроцедура: неправильный разделитель" + слово_раздел.стрИсх

	def __Двоеточие_Обрезать(сам)->bool:
		"""
		Здесь может быть, а может и не быть ":"
		"""
		бДвоеточие :bool= False
		слово_двоеточ :тСтрока= сам.слова_секции[0]
	...
	def __Возврат_Проверить(сам)->bool:
		"""
		Проверяет тип возврата процедуры. Должно быть разрешённой строкой.
		Кроме того, имя может быть составным
		"""
		стрОш :str= "тПроцедура: тип возврата должен быть допустимым именем"
		слово_возврат :тСтрока= сам.слова_секции[0]
		строка_возврат :str= слово_возврат.Проверить()
	...
	def __Процедура_Проверить(сам)->None:
		"""
		Проверяет наличие слова PROCEDURE среди слов секции
		"""
		слово_проц :тСтрока= сам.слова_секции[0]
		строка_проц :str= слово_проц.Проверить()
		бУсл :bool= строка_проц == "PROCEDURE"
		стрОш :str= "тПроцедура: процедура должна начинаться с PROCEDURE" + слово_проц.стрИсх
		assert бУсл, стрОш
		сам.СловаСекции_Обрезать()

	def __СкобкаЛев_Открыть(сам)->bool:
		"""
		...
		"""
		бРезультат :bool= False
		слово_скобка :тСтрока= сам.слова_секции[0]
		строка_скобка :str= слово_скобка.Проверить()
		if строка_скобка == "(":
			бРезультат = True
			сам.СловаСекции_Обрезать()
		return бРезультат

	def __СкобкаПрав_Закрыть(сам)->None:
		"""
	...
	def __Параметры_Проверить(сам)->None:
		"""
		Получает параметры процедуры.
		Крутит до тех пор, пока не станет ")"
		"""
		слово_закрыть :тСтрока= сам.слова_секции[0]
		строка_закрыть :str= слово_закрыть.Проверить()
		while (строка_закрыть != ")"): # есть зарытие параметров
			...
			параметр = None
			параметр :тПроцПараметр= тПроцПараметр(парам)
			сам.параметры[len(сам.параметры)] = параметр
			сам.слова_секции = {}
			сам.слова_секции = параметр.слова_секции

			слово_закрыть :тСлово= сам.слова_секции[0]
			строка_закрыть :str= слово_закрыть.Проверить()
