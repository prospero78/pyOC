	...
	def __Модуль_Контроль(сам)->None:  # Проверить правильность объявления модуля.
		...
		def НачалоМодуль_Проверить()->None:
			...
			if слово.строка != "MODULE":
				стрСообщ = "тМодуль: В файле отсутствует MODULE" + слово.стрИсх
				сам.__конс.Ошибка(стрСообщ)
		...
		def НачалоРазделитель_Проверить()->None:
			...
			if type(слово) != тСлово:
				...
				сам.__конс.Ошибка(стрСообщ)
			if слово.строка != ";":
				стрСообщ = "Нарушение окончания имени модуля"+слово.стрИсх
				сам.__конс.Ошибка(стрСообщ)
			сам.__СловаМодуля_Обрезать()
		def КонецМодуль_Найти()->None:
			...
			while цСчётОбр >= 0:
				...
				if type(слово_точка) != тСлово:
					стрСообщ =  "тМодуль: Слово должно быть тСлово, type="+str(type(слово_точка))
					сам.__конс.Ошибка(стрСообщ)
				# Между ними -- возможно имя модуля
				слово_конец :тСлово= сам.слова_модуля[цСчётОбр-2]
				...
				if стрТочка == "." and стрКонец == "END":
					...
					if стрИмя == сам.__имя:
						...
						стрСообщ = "тМодуль: Имя модуля не совпадает, модуль="+сам.__имя + \
									слово_имя.стрИсх
						сам.__конс.Ошибка(стрСообщ)
				цСчётОбр -= 1

			if цСчётОбр == -1:
				стрСообщ = "тМодуль: Нет окончания модуля "+сам.__имя
				сам.__конс.Ошибка(стрСообщ)
		...
		def МодульОдин_Проверить():
			...
			for номер_тега in сам.слова_модуля:
				...
				if слово.строка == "MODULE":
					счётчик_модуль += 1
					if счётчик_модуль > 1:
						стрСообщ = "тМодуль: MODULE в файле должно быть только один раз!"+ слово.стрИсх
						сам.__конс.Ошибка(стрСообщ)
