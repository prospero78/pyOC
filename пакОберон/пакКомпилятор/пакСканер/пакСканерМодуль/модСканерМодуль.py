...
class тСканерМодуль(тСекцияМодуль):
	...
	def __Модуль_Контроль(сам)->None:  # Проверить правильность объявления модуля.
		...
		def НачалоМодуль_Проверить()->None:
			...
			assert type(слово) == тСлово, "Слово должно быть тСлово, type="+str(type(слово))
		...
		def КонецМодуль_Найти()->None:
			...
			while цСчётОбр >= 0:
				...
				assert type(слово_точка) == тСлово, "тМодуль: Слово должно быть тСлово, type="+str(type(слово_точка))
