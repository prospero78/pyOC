	...
	def __Модуль_Контроль(сам)->None:  # Проверить правильность объявления модуля.
		...
		def НачалоМодуль_Проверить()->None:
			...
			if type(слово) != тСлово:
				стрСообщ = "тСканерМодуль: Слово должно быть тСлово, type="+str(type(слово))
				сам.__конс.Ошибка(стрСообщ)
			if слово.строка != "MODULE":
				стрСообщ = "тСканерМодуль: В файле отсутствует MODULE" + слово.стрИсх
				сам.__конс.Ошибка(стрСообщ)
		...
		def Имя_Проверить()->None:
			...
			if not слово.ЕслиИмя_Строго():
				стрСообщ =  "тСканерМодуль: Нарушение имени модуля"+слово.стрИсх
				сам.__конс.Ошибка(стрСообщ)
			# запомним имя модуля
		...
		def НачалоРазделитель_Проверить()->None:
			...
			if type(слово) != тСлово:
				...
				сам.__конс.Ошибка(стрСообщ)
			if слово.строка != ";":
				стрСообщ = "тСканерМодуль: Нарушение в окончании имени модуля"+слово.стрИсх
				сам.__конс.Ошибка(стрСообщ)
			сам.__СловаМодуля_Обрезать()
		...
		def КонецМодуль_Найти()->None:
			...
			while цСчётОбр >= 0:
				...
				if type(слово_точка) != тСлово:
					стрСообщ =  "тСканерМодуль: Слово должно быть тСлово, type="+str(type(слово_точка))
					сам.__конс.Ошибка(стрСообщ)
				# Между ними -- возможно имя модуля
				цСловоКонец = цСчётОбр-2
				if цСловоКонец < 0:
					сам.__конс.ОшВнутр("тСканерМодуль: выход за пределы словаря слов")
					return
				...
				if стрТочка == "." and стрКонец == "END":
					...
					if стрИмя == сам.__имя:
						...
						стрСообщ = "тСканерМодуль: Имя модуля не совпадает, модуль=\""+сам.__имя + \
									"\" " + слово_имя.стрИсх
						сам.__конс.Ошибка(стрСообщ)
				цСчётОбр -= 1
		...
		def МодульОдин_Проверить():
			...
			for номер_тега in сам.слова_модуля:
				...
				if type(слово) != тСлово:
					стрСообщ = "тСканерМодуль: Слово должно быть тСлово, type="+type(слово)
					сам.__конс.Ошибка(стрСообщ)
				if слово.строка == "MODULE":
					счётчик_модуль += 1
					if счётчик_модуль > 1:
		...
		if сам.__конс.бОшибка:
			сам.__конс.Ошибка("тСканерМодуль: ошибка в проверке разделителя модуля")
			return
		КонецМодуль_Найти()
