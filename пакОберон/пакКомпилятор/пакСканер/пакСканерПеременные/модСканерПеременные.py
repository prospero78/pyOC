...
class тСканерПеременные(тСекцияПерем):
	...
	def Элемент_Проверить(сам, пПоле)->str:
		...
		"""
		стрОш :str= "тСканерПеременные: тип элемента переменной должен быть допустимым именем, имя="
		слово_имя :тСлово= сам.слова_секции[0]
		имя :str= слово_имя.Проверить()

		assert слово_имя.ЕслиСтр_Допустимо(), стрОш + имя
		assert пПоле.поле.массив_тип == тРод.сБезТипа, "тСканерПеременные: элемент массива уже назначен, элемент=" + пПоле.поле.массив_тип
		элемент :str= ""
