...
class тСканерИсходник:
	def __init__(сам, пОберон:str, пИмяФайла : str)->None:
		...
		# признак того, что все слова выделены
		сам.__бСловаГотово = False
	...
	def __НаСлова_Разделить(сам)->None:
		"""
		В цикле делит всё на слова за один вызов.
		"""
		while not сам.__бСловаГотово:
			сам.__Слово_Выделить()

	def __Слово_Выделить(сам):
		"""
		Выделяет одно слово из исходного текста.
		"""
	...
	def Обработать(сам)->None:
		"""
		Выполняет обработку в пакетном режиме (не по шагам).
		"""
	...
	@property
	def бСловоГотово(сам):
		return сам.__бСловоГотово

	def Шаг(сам):
		"""
		Сканирует исходный текст по шагам.
		"""
		if not сам.__бСловоГотово:
			сам.__СловоВыделить()
