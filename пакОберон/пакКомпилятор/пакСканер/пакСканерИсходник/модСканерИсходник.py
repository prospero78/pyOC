...
class тСканерИсходник:
	def __init__(сам, пИмяФайла : str)->None:
		...
		сам.__исх  :тСканерИсхТекст = тСканерИсхТекст(пИмяФайла)   # хранит текст исходного кода
		сам.строки :тСканерИсхСтроки = тСканерИсхСтроки(сам.__исх())# хранит строки исходного текста
		сам.слова_модуля :dict = {}      # словарь слов в тексте
		сам.коорд :тКоордИзм= тКоордИзм(1, 0) # общая позиция в исходном тексте
	...
	def __Слово_Добав(сам, пСлово : тСлово)->None:
		...
		"""
		слово :тСлово= тСлово(сам, пСлово)
	...
	def Комментарии_Выкинуть(сам)->None:
		...
		def Коммент_Выкинуть():
			...
			бУсл :bool= лцУровень == 0
			стрОш :str= "Открытие и закрытие комментариев в модуле не сбалансировано, лцУровень=" + str(лцУровень)
			assert бУсл, стрОш
