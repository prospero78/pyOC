...
class тСканерКонстанты(тСекцияКонст):
	...
	def __ЕслиСекцияНеПусто(сам)->bool:
		...
		слово :тСлово= сам.слова_модуля[0]
		assert type(слово) == тСлово, "тКонст: Слово должно быть тСлово, type="+str(type(слово))
	...
	def __ЕслиСекцияОграничен(сам)->None:
		...
		while (not бМаркер) and (цСловаСчёт < (len(сам.слова_модуля))):
			...
			assert type(слово) == тСлово, "тКонстанты: Слово должно быть тСлово, type="+str(type(слово))
		...
		if слово.строка != ";":
			...
			assert False, "тКонстанты: слово должно быть ';'"+слово.строка+слово.стрИсх
