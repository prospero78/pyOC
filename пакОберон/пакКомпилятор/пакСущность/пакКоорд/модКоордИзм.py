...
class тКоордИзм(тКоордФикс):
	...
	def цСтр(сам, пцСтр:int):
		бУсл = type(пцСтр) == int
		стрСообщ = "тКоордИсх.Стр_Уст(пцСтр): пцСтр должен быть числом"+str(type(пцСтр))
		сам.__конс.Проверить(бУсл, стрСообщ)

		бУсл = пцСтр >= 1
		стрСообщ = "тКоорд.Стр_Уст(): пцСтр должен быть 1 или больше, пцСтр="+str(пцСтр)
		сам.__конс.Проверить(бУсл, стрСообщ)
	...
	def цПоз(сам, пцПоз:int):
		бУсл = type(пцПоз) == int
		стрСообщ = "тКоордИсх.Поз_Уст(пцПоз): пцПоз должен быть числом "+str(type(пцПоз))
		сам.__конс.Проверить(бУсл, стрСообщ)

		бУсл = пцПоз >= 0
		стрСообщ = "тКоордИсх.Поз_Уст(пцПоз): пцПоз должен быть 0 или больше, пцПоз="+str(пцПоз)
		сам.__цПоз = пцПоз
