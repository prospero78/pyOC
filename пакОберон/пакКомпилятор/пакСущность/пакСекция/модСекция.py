...
if True:
	...
	from ..пакОшибка import тОшибка

class тСекция:
	__slots__ = ("ошс", "__конс", "__секция", "слова_модуля",\
					"слова_секции", "__слово_конец")
	def __init__(сам, пОберон, пДанные:dict)->None:
		def Слова_Проверить():
			if type(пДанные['слова']) != dict:
				...
				сам.ошс.Внутр("__init__()", стрОш)
				return

		сам.__конс = пОберон.конс
		сам.__конс.Отладить("тСекция.__init__()")

		сам.ошс = тОшибка(пОберон, "тСекция")
	...
	def СловаМодуля_Печать(сам):
		...
		"""
		сам.__конс.Печать("Слова модуля в секции " + сам.__секция)
	...
	def СловаСекции_Печать(сам):
		...
		"""
		сам.__конс.Печать("Слова секции в секции" + сам.__секция)
	...
	def Конец_Уст(сам, пСлово):
		...
		if сам.__слово_конец != None:
			стрОш = "__слово_конец уже установлено" + сам.__слово_конец.стрИсх
			сам.ошс.Внутр("Конец_Уст()", стрОш)
			return
		if type(пСлово) != тСлово:
			стрОш = "пСлово должно быть тСлово, type="+str(type(пСлово))
			сам.ошс.Внутр("Конец_Уст()", стрОш)
			return
