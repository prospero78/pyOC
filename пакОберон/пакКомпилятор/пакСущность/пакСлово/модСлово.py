...
class тСлово:
	...
	def __init__(сам, пОберон, пИсх, пСтрока):
		def пСтрока_Проверить():
			...
			if len(пСтрока) == 0:
				стрСообщ = "пСтрока не может быть пустым"
				сам.ош.Внутр("__init__()", стрОш)
				return
		def Род_Проверить():
			"""
		...
		if сам.коорд.ош.бВнутр:
			сам.ош.Внутр("__init__()", "При вызове тКоордФикс")
	...
	def строка(сам)->str:
		...
		if type(сам.__строка) != str:
			стрОш = "Строка исходника должна быть str, type="+str(type(сам.__строка))
			сам.ош.Внутр("строка", стрОш)
			return ""
		if сам.__строка == "":
			сам.ош.Внутр("строка", "Строка тСлово не может быть пустой")
			return ""
		return сам.__строка
