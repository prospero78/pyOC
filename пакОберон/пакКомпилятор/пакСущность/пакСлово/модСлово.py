...
class тСлово:
	...
	def __init__(сам, пОберон, пИсх, пСтрока):
		...
		def Род_Проверить():
			...
			if пСтрока == ";":
				...
				сам.__конс.Ошибка("тСлово.__init__(): не могу классифицировать строку, строка="+пСтрока)
		...
		if пСлово_Проверить():
			сам.__конс.ОшВнутр("тслово.__init__(): ошибка при проверке пСтрока")
			return
	...
	def стрИсх(сам)->str:
		...
		"""
		if сам.__стрИсх == "":
			стрСообщ = "тСлово.стрИсх(знач): Строка исходника не может быть пустой"
			сам.__конс.ОшВнутр(стрСообщ)
			return ""
	...
	def Проверить(сам)->str:
		...
		if type(сам.__строка) != str:
			стрСообщ = "тСлово.Проверить(): строка должна быть 'str', type=" + str(type(сам.__строка))
			сам.__конс.ОшВнутр(стрСообщ)
			return ""

		if сам.__строка == "":
			стрСообщ = "тСлово.Проверить(): строка не может быть пустой"
			сам.__конс.ОшВнутр(стрСообщ)
			return ""
