...
class тКомпилер:
	...
	def Выполнить(сам, пФайлИсхИмя:str):
		...
		if type(пФайлИсхИмя) != str:
			сам.ош.Внутр("Выполнить()", "пФайлИсхИмя должен иметь тип str, type="+str(type(пФайлИсхИмя)))
			return
		if пФайлИсхИмя == "":
			сам.ош.Внутр("Выполнить()", "пФайлИсхИмя не может быть пустой строкой")
			return
		сам.__конс.Отладить("Выполнить(): компилирую \"" + пФайлИсхИмя + "\"")

		сканер = тСканер(сам.__оберон, пФайлИсхИмя)
		if сканер.ош.бВнутр:
			сам.ош.Внутр("Выполнить()", "При создании тСканер")
			return
		if сканер.ош.бИсх:
			сам.ош.Исх("Выполнить()", "При создании тСканер")
			return

		анализ = тАнализ(сам.__оберон, сканер.Секции_Получить())
		if анализ.ош.бВнутр:
			сам.ош.Внутр("Выполнить()", "При создании тАнализ")
			return
		if анализ.ош.бИсх:
			сам.ош.Исх("Выполнить()", "При создании тАнализ")
			return

		разбор = тРазбор(анализ.данные)
		if разбор.ош.бВнутр:
			сам.ош.Внутр("Выполнить()", "При создании тРазбор")
			return
		if разбор.ош.бИсх:
			сам.ош.Исх("Выполнить()", "При создании тРазбор")
			return
