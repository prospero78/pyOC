# coding: utf8
"""
Предотавляет базовый тип поля-записи в секции типов.
"""
...
class тАнализТипПолеБазовый:
	def __init__(сам, пДанные):
		def Слова_Проверить():
			...
			assert бУсл, стрОш
			бУсл = пДанные['секция'] == "TYPE_FIELD"
			assert бУсл, "тАнализТипБазовый: ошибочное использование типа в TYPE_FIELD, секция=" + пДанные['секция']
	...
	def ИмяПоле_Проверить(сам):
		...
		print("тАнализТипПолеБазовый: 5501 Имя поля=", имя, слово_имя.стрИсх)
	...
	def __Род_Проверить(сам):
		...
		if строка_род in тип_встроен:
			...
			assert False, "тАнализТипПолеБазовый: Неизвестный род типа, род="+строка_род
	...
	def Разделитель_Обрезать(сам):
		"""
		В полях последнее слово ";" или "END"
		Поэтому первое необходимо обрезать, а второе пропустить
		"""
		слово_раздел = сам.слова_секции[0]
		if слово_раздел.род == тСлово.кТочкаЗапятая:
			сам.СловаСекции_Обрезать()
		elif слово_раздел.строка == "END":
			pass # обработает владелец верхнего блока
		else:
			# В типе всегда есть разделитель в конце
			assert False, "тАнализТипПолеБазовый: неправильный разделитель типа" + слово_раздел.стрИсх
	...
	def Предок_Проверить(сам):
		...
		while (имя == ".") or (слово_имя.ЕслиСтр_Допустимо()):
			...
			слово_имя = сам.слова_секции[0]
			имя = слово_имя.Проверить()
		print("тАнализТипПолеБазовый: предок=",слово_имя.строка, слово_имя.стрИсх)
	...
	def бЭкспорт_Проверить(сам):
		...
		print("тАнализТипПолеБазовый: 6798 экспорт поля=", сам.__бЭкспорт)
