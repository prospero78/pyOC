...
if True:
	...
	from .пакАнализТипЗапись import тАнализЗаписьПоле
...
class тАнализТипЗапись(тАнализТипБазовый):
	...
	def __Поля_Проверить(сам):
		...
		if not (строка_конец != "END"): # если запись не пустая
			...
			while цСчётКонец > 0: # нет окончания описания типа
				парам = {}
				парам['секция'] = "TYPE" # Анализ полей записи в секции TYPE
				парам['слова']  = сам.слова_секции
				print("test 8467")
				сам.поля[len(сам.поля)] = тАнализЗаписьПоле(парам)
	...
	def __КОНЕЦ_Обрезать(сам) -> bool:
		...
		if строка_конец == "END": # есть окончание
			сам.СловаСекции_Обрезать()
		elif слово_конец.ЕслиИмя_Строго():
			pass # значит тип записи имеет поля
			print("тАнализТипЗапись: 0902 обнаружены поля записи")
		else:
