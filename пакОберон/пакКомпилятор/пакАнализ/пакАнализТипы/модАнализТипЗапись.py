...
class тАнализТипЗапись(тАнализТипБазовый):
	def __init__(сам, пДанные):
		...
		сам.Имя_Проверить()
		print("тАнализТипЗапись: имя типа записи=", сам.имя)
		сам.бЭкспорт_Проверить()
		print("тАнализТипЗапись: экспорт типа записи=", сам.бЭкспорт)
		сам.Определитель_Проверить()
		сам.бУказатель_Проверить()
		print("тАнализТипЗапись: указатель типа записи=", сам.бСсылка)
	...
	def __Поля_Проверить(сам):
		...
		while True: # пока идёт разбор записи
			if строка_конец == "END":
				...
				break
			парам = {}
			парам['секция'] = "TYPE_FIELD" # Анализ полей RECORD в секции TYPE
			...
			слово_конец = сам.слова_секции[0]
			строка_конец = слово_конец.Проверить()
		print("тАнализТипЗапись ============================== конец записи ?????? ",слово_конец.стрИсх)
	...
	def __ЕслиКОНЕЦ_Обрезать(сам) -> bool:
		...
		слово_конец = сам.слова_секции[0]
		строка_конец = слово_конец.Проверить()
		print("тАнализТипЗапись: ############ конец?", строка_конец)
