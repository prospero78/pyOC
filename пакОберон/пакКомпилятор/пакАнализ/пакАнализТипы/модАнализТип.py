...
class тАнализТип(тАнализТипБазовый):
	def __init__(сам, пОберон, пДанные):
		...
		сам.__конс.Отладить("тАнализТип.__init__()")
		...
		if сам.бОшВнутр_АнализТипБазовый:
			сам.__бОшВнутр = True
			стрОш = "тАнализТип.__init__(): ошибка компилятора. При вызове тАнализБазовый"
			сам.__конс.ОшВнутр(стрОш)
			return
		if сам.бОшИсх_АнализТипБазовый:
			сам.__бОшИсх = True
			стрОш = "тАнализТип.__init__(): ошибка исходника. При вызове тАнализБазовый"
		...
		if len(сам.слова_секции) < 2:
			сам.__бОшВнутр = True
			стрОш = "тАнализТип.__init__(): ошибка компилятора. Неполное определение секции"
			сам.__конс.ОшВнутр(стрОш)
			return
	...
	def __Имя_Проверить(сам):
		...
		if слово_имя.ЕслиИмя_Строго():
			...
			сам.__бОшИсх = True
			стрОш = "тАнализТип: ошибка исходника. Имя типа должно быть допустимым именем" + слово_имя.стрИсх
			сам.__конс.Ошибка(стрОш)
			return
	...
	def __Экспорт_Проверить(сам):
		...
		if строка_экспорт == "*": # есть экспорт
			...
			сам.__бОшИсх = True
			стрОш =  "тАнализТип: ошибка исходника. Символ экспорта допустим '*' или '='" + слово_экспорт.стрИсх
			сам.__конс.Ошибка(стрОш)
			return
	...
	def __Определитель_Проверить(сам):
		...
		if слово_опр.род == тСлово.кРавно: # правильное выражение определения типа
			...
			сам.__бОшИсх = True
			стрОш = "тАнализТип: ошибка исходника. Отсутствует определитель \"=\" в объявлении типа" + слово_опр.стрИсх
			сам.__конс.Ошибка(стрОш)
			return
	...
	def __Указатель_Проверить(сам):
		...
		if строка_указ == "POINTER": # модификатор типа, а не сам тип
			...
			if строка_из != "TO":
				сам._бОшИсх = True
				стрОш = "тАнализТип: ошибка исходника. За POINTER не следует TO" + слово_из.стрИсх
				сам.__конс.Ошибка(стрОш)
				return
			#сам.СловаСекции_Обрезать()
	...
	def __ЧастныйПредок_Проверить(сам, пСлово):
		...
		if type(пСлово) != тСлово:
			сам.__бОшВнутр = True
			стрОш = "тАнализТип: ошибка компилятора. пСлово должно быть тСлово, type="+str(type(пСлово))
			сам.__конс.ОшВнутр(стрОш)
			return
		if not пСлово.ЕслиСтр_Допустимо():
			сам.__бОшИсх = True
			стрОш = "тАнализТип: ошибка исходника. Имя типа должно быть допустимым именем "+ стрОш + пСлово.строка + пСлово.стрИсх
			сам.__конс.Ошибка(стрОш)
			return
		if сам.__предок != тРод.сБезТипа:
			сам.__бОшВнутр = True
			стрОш = "тАнализТип: ошибка компилятора. Предок уже назначен, предок=" + сам.__предок+ слово_имя.стрИсх
			сам.__конс.ОшВнутр(стрОш)
			return
		сам.__предок = ""
	...
	def __Род_Проверить(сам):
		...
		if not слово_тип.ЕслиСтр_Допустимо():
			сам.__бОшВнутр = True
			стрОш = "тАнализТип: ошибка компилятора. Род типа должен быть допустимым именем" + слово_тип.стрИсх
			сам.__конс.ОшВнутр(стрОш)
			return
		if строка_род in тРод.тип_встроен: # текущий тип основан на встроенном типе
			сам.тип = тАнализТипВстроен(сам.__оберон, парам)
			if сам.тип.бОшВнутр:
				сам.__бОшВнутр = True
				стрОш = "тАнализТип: ошибка компилятора. При вызове тАнализТипВстроен " + слово_тип.стрИсх
				сам.__конс.ОшВнутр(стрОш)
				return
			if сам.тип.бОшИсх:
				сам.__бОшИсх = True
				стрОш = "тАнализТип: ошибка исходника. При вызове тАнализТипВстроен " + слово_тип.стрИсх
				сам.__конс.Ошибка(стрОш)
				return
		elif строка_род == тРод.сМассив: # текущий тип основан на массиве
			сам.тип = тАнализТипМассив(сам.__оберон, парам)
			if сам.тип.бОшВнутр == True:
				сам.__бОшВнутр = True
				стрОш = "тАнализТип: ошибка компилятора. При вызове тАнализТипМассив " + слово_тип.стрИсх
				сам.__конс.ОшВнутр(стрОш)
				return
			if сам.тип.бОшИсх == True:
				сам.__бОшИсх = True
				стрОш = "тАнализТип: ошибка исходника. При вызове тАнализТипМассив " + слово_тип.стрИсх
				сам.__конс.Ошибка(стрОш)
				return
		elif строка_род == тРод.сЗапись: # текущий тип основан на записе
			сам.тип = тАнализТипЗапись(сам.__оберон, парам)
			if сам.тип.бОшВнутр == True:
				сам.__бОшВнутр = True
				стрОш = "тАнализТип: ошибка компилятора. При вызове тАнализТипЗапись " + слово_тип.стрИсх
				сам.__конс.ОшВнутр(стрОш)
				return
			if сам.тип.бОшИсх == True:
				сам.__бОшИсх = True
				стрОш = "тАнализТип: ошибка исходника. При вызове тАнализТипЗапись " + слово_тип.стрИсх
				сам.__конс.Ошибка(стрОш)
				return
		elif строка_род == "PROCEDURE": # текущий тип основан на процедуре
			сам.тип = тАнализТипПроцедура(сам.__оберон, парам)
			if сам.тип.бОшВнутр == True:
				сам.__бОшВнутр = True
				стрОш = "тАнализТип: ошибка компилятора. При вызове анализа PROCEDURE " + слово_тип.стрИсх
				сам.__конс.ОшВнутр(стрОш)
				return
			if сам.тип.бОшИсх == True:
				сам.__бОшИсх = True
				стрОш = "тАнализТип: ошибка исходника. При вызове анализа PROCEDURE " + слово_тип.стрИсх
				сам.__конс.Ошибка(стрОш)
				return
		elif слово_тип.ЕслиСтр_Допустимо(): # Пользовательский тип
			сам.тип = тАнализТипПерем(сам.__оберон, парам)
			if сам.тип.бОшВнутр == True:
				сам.__бОшВнутр = True
				стрОш = "тАнализТип: ошибка компилятора. При вызове тАнализТипПерем " + слово_тип.стрИсх
				сам.__конс.ОшВнутр(стрОш)
				return
			if сам.тип.бОшИсх == True:
				сам.__бОшИсх = True
				стрОш = "тАнализТип: ошибка исходника. При вызове тАнализТипПерем " + слово_тип.стрИсх
				сам.__конс.Ошибка(стрОш)
				return
		else:
			сам.__бОшВнутр = True
			стрОш = "тАнализТип: ошибка компилятора. Неизвестный тип записи" + слово_тип.стрИсх
			сам.__конс.ОшВнутр(стрОш)
			return
		if сам.тип == None:
			сам.слова_секции = {}
	...
	def Паспорт_Печать(сам):
		сам.__конс.Печать("+ тАнализТип: имя_типа=", сам.имя)
		сам.__конс.Печать("|      экспорт =", сам.тип.бЭкспорт)
		сам.__конс.Печать("|    указатель =", сам.тип.бУказатель)
		сам.__конс.Печать("|       предок =", сам.тип.предок)
		if сам.тип.предок == тРод.сМассив:
			for ключ in range(len(сам.тип.массив_размерность)):
				сам.__конс.Печать("|      ключ ", ключ, ":",сам.тип.массив_размерность[ключ])
			сам.__конс.Печать("|      элем    =", сам.тип.элем)
