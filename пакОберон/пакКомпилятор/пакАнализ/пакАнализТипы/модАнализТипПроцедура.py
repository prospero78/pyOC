...
class тАнализТипПроцедура(тАнализТипБазовый):
	def __init__(сам, пОберон, пДанные):
		...
		сам.ош = тОшибка(пОберон, "тАнализТипПроцедура")

		тАнализТипБазовый.__init__(сам, пОберон, пДанные)
		if сам.ошб.бВнутр:
			стрОш = "тАнализПроцедура.: ошибка компилятора. При вызове тАнализТипБазовы"
		...
		if сам.__СкобкаОткр_Проверить():
			while сам.слова_секции[0].строка != ")":
				...
				if параметры.ош.бВнутр:
					сам.ош.Внутр("__init__()", "При создании тАнализТипПарам")
					return
				if параметры.ош.бИсх:
					сам.ош.Исх("__init__()", "При создании тАнализТипПарам")
					return
	...
	def __СловоПроцедура_Проверить(сам):
		...
		строка_проц = слово_проц.строка
		if строка_проц != "PROCEDURE":
			стрОш = "Отсуствует кючевое слово 'PROCEDURE'" + слово_проц.стрИсх
			сам.ош.Исх("__СловоПроцедура_Проверить()", стрОш)
			return
		else:
	...
	def __СкобкаОткр_Проверить(сам) -> bool:
		...
		строка_скоб = слово_скобка.строка
	...
	def __ИмяПарам_Проверить(сам):
		...
		if слово_имя.ЕслиИмя_Строго():
			...
			стрОш = "Имя параметра недопустимо" + слово_имя.стрИсх
			сам.ош.Исх("__ИмяПарам_Проверить()", стрОш)
			return
		return рез
	...
	def __Скобка_Закрыть(сам):
		...
		if строка_скоб == ")": # начало параметров в процедуре
			...
			стрОш ="В параметрах типа-процедуры должна быть скобка закрытия ')'"+ слово_скобка.стрИсх
			сам.ош.Исх("__Скобка_Закрыть()", стрОш)
			return
		return бРезульт
	...
	def __Разделитель_Обрезать(сам):
		...
		"""
		слово_имя = сам.слова_секции[0]
		строка_раздел = слово_имя.строка
		if строка_раздел == ";":
			сам.СловаСекции_Обрезать()
		elif строка_раздел == ")":
			pass # окончание слов параметров, не надо обрезать
		else:
			стрОш = "Неправильный разделитель поля" + слово_имя.стрИсх
			сам.ош.Исх("__Разделитель_Обрезать()", стрОш)
			return
	...
	def __Возврат_Проверить(сам):
		...
		стрРезульт = слово_резузльт.строка
		if стрРезульт == "BOOLEAN":
			...
			стрОш = "Возвращаемый тип должен быть встроенным"+слово_резузльт.стрИсх
			сам.ош.Исх("__Возврат_Проверить()", стрОш)
			return
		if сам.результ == "":
			сам.ош.Исх("__Возврат_Проверить()", "Тип параметра не может быть пустой строкой")
			return
		сам.СловаСекции_Обрезать()
	...
	def __Двоеточие_Обрезать(сам):
		...
		if строка_двоеточ == ":": # есть двоеточие
			...
			стрОш = "Разделитель должен быть ':'" + слово_двоеточ.стрИсх
			сам.ош.Исх("__Двоеточие_Обрезать()", стрОш)
			return
