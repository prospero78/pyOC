...
class тКонстПоле:
	def __init__(сам, пОберон, пДанные):
		...
		if пДанные['секция'] != "CONST":
			сам.__бОшВнутр = True
			стрСообщ = "тКонстПоле.__init__(): ошибка компилятора. Использование типа не в своей секции, секцйия=" + пДанные['секция']
		...
		if сам.__Имя_Проверить():
			сам.__бОшВнутр = True
			сам.__конс.ОшВнутр("тКонстПоле.__init__(): ошибка компилятора. При проверке имени константы")
			return
		if сам.__Экспорт_Проверить():
			сам.__бОшВнутр = True
			сам.__конс.ОшВнутр("тКонстПоле.__init__(): ошибка компилятора. При проверке экспорта")
			return
		...
		if сам.__конст.бОшВнутр:
			сам.__бОшВнутр= True
			стрОш = "тКонстПоле.__init__(): ошибка компилятора. При вызове тКонстанта"
	...
	def __Имя_Проверить(сам)->bool:
		...
		if type(сам.__имя) == None:
			сам.__бОшИсх = True
			стрОш = "тКонстПоле: ошибка исходника. Имя константы уже присвоено" + сам.__имя.стрИсх
		...
		if слово_имя.ЕслиИмя_Строго():
			...
			if not слово_имя.ЕслиИмя():
				сам.__бОшИсх = True
				стрСообщ = "тКонстПоле: ошибка исходника. Имя поля должно быть допустимым именем" + слово_имя.стрИсх
	...
	def __Экспорт_Проверить(сам)->bool:
		...
		if type(сам.__бЭкспорт_слово) == None:
			сам.__бОшИсх = True
			стрСообщ = "тКонстПоле: ошибка исходника. Экспорт уже присвоен"+сам.__бЭкспорт_слово.стрИсх
			сам.__конс.Ошибка(стрМообщ)
			return True
		if слово_экспорт.род == тСлово.кУмножить: # есть экспорт
			...
			сам.__бОшИсх = True
			сам.__конс.Ошибка("тКонстПоле: ошибка исходника. Символ экспорта допустим '*' или '='" + слово_экспорт.стрИсх)
			return
	...
	def __Проверить_Равно(сам):
		...
		if слово_равно.род == тСлово.кРавно: # есть уравнивание
			...
			сам.__бОшИсх = True
			сам.__конс.Ошибка("тКонстПоле: ошибка исходника. Литера приравнивания должен быть '='" + слово_равно.стрИсх)
			return
